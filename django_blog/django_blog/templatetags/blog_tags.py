...
from django_blog.django_blog.models import Post, Tags, Menu
from django_blog.django_blog.views import get_user_role
...
def get_archives(context):
    archives = []
    dates = []
    for each_object in Post.objects.filter(category__is_active=True, status="Published").order_by('created_on').values('updated_on'):
